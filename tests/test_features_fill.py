...
age_and_class_small = {1: [np.nan, np.nan], 2: [33, np.nan], 3: [18, np.nan]}
expected_small = {1: [25, 25], 2: [33, 33], 3: [18, 18]}
...
def make_df_from_dict(input_dict: dict) -> pd.DataFrame:
    ...
    df = pd.DataFrame({"Pclass": pclass, "Age": ages_on_board})
...
@pytest.mark.parametrize(
    ("age_and_class", "expected"),
    [({1: [np.nan]}, 'The "Age" column is fully missed'), ({}, "df is empty")],
)
